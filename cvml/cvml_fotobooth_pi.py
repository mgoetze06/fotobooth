...
def handDetection(thumbs_img,debug,allClasses,classNames,interpreter,isCV2Img,confidence):
    global pi_Used
    ...
    hands = mpHands.Hands(static_image_mode=False,max_num_hands=1, min_detection_confidence=confidence)
    ...
    #framergb = cv2.cvtColor(framergb, cv2.COLOR_BGR2GRAY)
...
def handDetectionHandler(img,showImg):
    ...
    if img.endswith(".jpg") or img.endswith(".JPG") or img.endswith(".png"):
        ...
        detection, thumbs_img = handDetection(img, debug=debug, allClasses=True, interpreter=interpreter,classNames=classNames,isCV2Img=True,confidence=0.4)        #detection, thumbs_img = handDetection(img, debug=debug, allClasses=True, model=model, classNames=classNames)
        #detection,thumbs_img = handDetection('thumbsup2.jpg',debug=True,allClasses=False)
        zoom_scale = 0.05
        h,w = 700,500
        while len(detection) == 0:
            newimg = img[int(zoom_scale * w):int((1 - zoom_scale) * w), int(zoom_scale * h):int((1 - zoom_scale) * h)]
            img = cv2.resize(newimg,(700,500))
            detection, thumbs_img = handDetection(img, debug=debug, allClasses=True, interpreter=interpreter,classNames=classNames,isCV2Img=True,confidence=0.4)
            if showImg:
                cv2.imshow("window", thumbs_img)
                cv2.waitKey()
        ...
        try:
            ...
            print("result not deleted")
        result = [["detectiontime [s]", "filename", "number of detected hands", "predicted label", "label prob","true label"]]
        ...
        if os.path.exists('test.npy'):
            ...
            with open('test.npy', 'rb') as f:
                ...
                #print(temp)
                try:
                    print("trying to append result to existing test.npy") 
                    result = np.append(temp,result,axis=0)
                except:
                    print("result not appended to existing test.npy") 
                    pass
                f.close()
        ...
        for file in files:
            ...
            confidence = 0.4
            detection, thumbs_img = handDetection(img, debug=debug, allClasses=False,interpreter=interpreter,classNames=classNames,isCV2Img=True,confidence=confidence)        #detection, thumbs_img = handDetection(img, debug=debug, allClasses=True, model=model, classNames=classNames)
            endtime = time.time()
            print("detection time: ",endtime -starttime)
            try_img = 0
            #print(detection)
            ...
            h,w = 700,500
            try_img = 0
            while len(detection) == 0 and try_img < 3:
                starttime = time.time()
                newimg = img[int(zoom_scale * w):int((1 - zoom_scale) * w), int(zoom_scale * h):int((1 - zoom_scale) * h)]
                img = cv2.resize(newimg,(h,w))
                detection, thumbs_img = handDetection(img, debug=debug, allClasses=False, interpreter=interpreter,classNames=classNames,isCV2Img=True,confidence=0.4)
                endtime = time.time()
                try_img += 1

            print("detection while loop done")
            print(filename)
            ...
            if not len(detection) > 0:
                ...
                #res_obj = [[str(round(endtime -starttime,3)),filename,len(detection),"nothing found","0",truelabel]]
                ...
                res_obj = np.array(res_obj)
                result = np.append(result,res_obj,axis=0)
            #result.append(res_obj)
            if showImg:
                cv2.imshow("window", thumbs_img)
                cv2.waitKey()
            if (detection_counter % 5 == 0) and (detection_counter > 1):
                print("I'm saving the array ...")
                f = open("files.txt", 'w+')
                ...
                with open('test.npy', 'wb') as f:
                    ...
                    #    result = np.array(result)
                    #except:
                    #    print("result could not be deleted")
                    #    pass
                    
                #for res in result: print(res)
            else:
                print("detetion counter: ",detection_counter)
...
if __name__ == '__main__':
    ...
    #handDetectionHandler("/home/pi/cvml/programs/metricsTest/thumbs up_test10.JPG", showImg=True)
    handDetectionHandler("/home/pi/cvml/programs/metricsTest/", showImg=False)
    #handDetectionHandler("/home/pi/cvml/programs/test.png", showImg=True)
